...
int main(void)
{
	...
	while(1){
		...
		I2C_MasterTx(&hI2C1, &commandcode, 1, SLAVE_ADDR, I2C_REPEAT_START);

		I2C_MasterRx(&hI2C1, &len, 1, SLAVE_ADDR, I2C_REPEAT_START);

		commandcode = 0x52;

		I2C_MasterTx(&hI2C1, &commandcode, 1, SLAVE_ADDR, I2C_REPEAT_START);

		I2C_MasterRx(&hI2C1, rcvbuff, len, SLAVE_ADDR, I2C_NO_REPEAT);
